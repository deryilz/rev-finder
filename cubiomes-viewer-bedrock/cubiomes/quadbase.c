...
#ifdef USE_PTHREAD
static void *searchAll32Thread(void *data)
#else
static DWORD WINAPI searchAll32Thread(LPVOID data)
#endif
{
// TODO TEST:
        ...
        )
{
    ...
    for (t = 0; t < threads; t++)
    {
        info[t].start = (t * (MASK32+1) / threads);
        info[t].end = ((t+1) * (MASK32+1) / threads - 1);
    ...
    for (t = 0; t < threads; t++)
    {
        pthread_create(&tids[t], NULL, searchAll32Thread, (void*)&info[t]);
    }
    ...
    for (t = 0; t < threads; t++)
    {
        tids[t] = CreateThread(NULL, 0, searchAll32Thread,
